...
Let's build a meme generator block. It pulls meme templates from a third-party API, lets people add their own text, and registers some custom fonts so the text actually looks like a meme. It's a fun one, and there are a couple of useful tricks along the way, like keeping a `fetch` call from running on every render.

## Setup

The scaffold gives you a working block with the attributes and helper components already in place, so you can spend your time on the interesting parts.

...
So in this recipe, we're building a meme generator block. It pulls popular templates from a third-party meme API and lets people add their own text on top. Then we'll register some custom fonts, because a meme in the wrong font just doesn't land. This one's a lot of fun.
...
Memes live and die by their images, and I didn't want to hand-curate a list. Imgflip has a public endpoint that returns the most popular meme templates, so we'll lean on that. The request itself is the easy part. The tricky bit is making sure React doesn't fire it a few thousand times.
...
Part of what makes memes great is the font. Nobody takes a meme seriously in Times New Roman. A plugin can't install fonts for you, but it can offer a collection that people install in a couple of clicks, and I actually think that's the nicer experience anyway.

...
Let's build a meme generator block. It pulls templates from a third-party meme API and lets people add their own text. Then we'll register some custom fonts, because the caption has to look right or the joke doesn't land. This one's a lot of fun, so grab your spatula.

## Setup

The scaffold gives you a working block with the attributes and helper components already in place, so you can skip the boilerplate and get to the interesting stuff.
...
A meme is only as good as its image, and I didn't want to hand-curate a list. Imgflip has a public endpoint that returns the most popular meme templates, so we'll lean on that. The request itself is easy. What you have to watch out for is React firing it a few thousand times.
...
Part of what makes memes great is the font. Nobody takes a meme seriously in Times New Roman. A plugin can't install fonts for you, but it can offer a collection that people install in a couple of clicks, and I think that's a nicer experience anyway.
...
Stick a fork in it, you're done! If you build something ridiculous with this, I want to see it.

...
| Repository | You want the development environment in the repository. This option needs Docker. |

## Standalone

Instructions
In a terminal, go to the `plugins` directory of your local WordPress installation. Then run this command:

```bash
npx @wordpress/create-block@latest meme-generator –template @block-developer-cookbook/meme-generator
```

When the scaffold is complete, run this command to start the build in the new plugin directory:

```bash
cd meme-generator && npm run start
```

In **Plugins > Installed Plugins**, activate **Meme Generator**.
...
Instructions
**CAUTION:** Do not continue if Docker is not installed and running. The development environment does not start without Docker.

If you do not have a copy of the repository, clone it:
...
In `block.json`, replace the file contents with this code:
...
**CAUTION:** Do not leave the editor open for a long time after this change. The code sends requests in a loop and can crash the browser.
...
In `src/edit.js`, remove the `fetch` call. A later step adds the call again.

Replace the `Edit` function with this code:
...
1. Save `src/edit.js`.
2. Refresh the editor.
3. Make sure that the console shows the `useEffect is running` message one time only.
...
1. Save `src/edit.js`.
2. Refresh the editor.
3. Make sure that the console shows the list of memes one time only.
...
The `color` settings do these things:

| Property | Value | Result |
|---|---|---|
| `text` | `true` | The user can select a text color from the colors of the active theme. |
| `background` | `false` | Removes the background color control. This control is on by default, but the block does not use it. |
| `enableContrastChecker` | `false` | Removes the contrast checker. The checker compares the text color to the theme background, not the image, so it can show false warnings. |

1. Refresh the editor.
2. Select the block.
3. Make sure that the block settings show a **Text** color control:
...
1. Refresh the editor.
2. Make sure that the block settings show controls for font size and text alignment:
...
1. Refresh the editor.
2. Make sure that the block settings show controls for font family, font style, and font weight:
...
3. Next to **Fonts**, select the **Manage fonts** icon.
4. Select the **Install Fonts** tab.
5. Make sure that the **Meme fonts** collection shows:

![screenshot](image)

6. Install the fonts that you want to use.
7. Make sure that the installed fonts show in the font family control of the block:
